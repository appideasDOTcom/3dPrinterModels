    ...
    {
        {
            translate( [(cutoutXY/2) - (wtOnTopBaseWidth/2), (cutoutXY + topPieceSupport), 0] )
            {
                ...
                translate( [-3.5 + (wtBasePlateCornerDiameter/2), 0, wtBasePlateCornerDiameter/2] )
                {
                    ...
                    {
                        {
                            cube( [(distanceBetweenTopNutTraps + 7 - wtBasePlateCornerDiameter), wtBaseHeight, (height/2) - (wtBasePlateCornerDiameter/2)] );
                        }
                        {
                            union()
                            {
                                translate( [(-1 * (wtBasePlateCornerDiameter/2)), 0, (height/2) - (wtBasePlateCornerDiameter)] )
                            {
                                cube( [(wtBasePlateCornerDiameter/2), wtBaseHeight, (wtBasePlateCornerDiameter/2)] );
                            }

                            translate( [(distanceBetweenTopNutTraps + 7 - wtBasePlateCornerDiameter), 0, (height/2) - (wtBasePlateCornerDiameter)] )
                            {
                                cube( [(wtBasePlateCornerDiameter/2), wtBaseHeight, (wtBasePlateCornerDiameter/2)] );
                            }

                            translate( [0, wtBaseHeight, 0] )
                            {
                                rotate( [90, 0, 0] )
                                {
                                    cylinder( d=3, h=wtBaseHeight );
                                }
                            }

                            translate( [(distanceBetweenTopNutTraps + 7 - wtBasePlateCornerDiameter), wtBaseHeight, 0] )
                            {
                                rotate( [90, 0, 0] )
                                {
                                    cylinder( d=3, h=wtBaseHeight );
