	...
	difference() {
		union() {
			difference() {
				...
				// The pocket the base wall drops into, ending exactly at the seam
				// so the remaining ring becomes the shoulder that carries the lid
		...
		// Cut last and through both, so the same pair of slots frees the finger
		// from the collar and sets its width in one operation
		_snap_slots();
	}
